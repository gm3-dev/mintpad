            ...
            <form method="POST" action="{{ route('collections.store') }}" enctype="multipart/form-data">
                ...
                <div class="w-full flex flex-wrap">
                    <div class="basis-1/3 mb-4">
                        <x-label for="symbol" :value="__('Blockchain')" class="relative is-required" info="Choose which blockchain you want to launch your NFT collection on." />
                        <x-select class="mt-1 !w-full" v-model="collection.chain_id" :options="$blockchains"></x-select>
                    </div>
                    <div class="basis-1/3 mb-4 px-2">
                        <x-label for="symbol" :value="__('Symbol / Ticker')" class="relative is-required" info="The symbol of the token contract is the symbol by which the token contract should be known, for example “DOGGY” or “BAYC”. It is broadly equivalent to a stock ticker." />
