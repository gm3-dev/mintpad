<x-mint-layout>
    ...
    <div v-if="fatalError" class="text-center mt-12 text-xl">
        <i class="fas fa-exclamation-triangle"></i> {{ __('Something went wrong') }} <i class="fas fa-exclamation-triangle"></i>
    </div>
    ...
    <div v-else-if="!fatalError">
        <div class="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-xl" v-bind:class="{'h-96': !collection.image}">
                <img v-if="collection.image" v-bind:src="collection.image" class="rounded-xl" />
                <p v-else class="text-center mt-44 text-sm text-mintpad-300">No image</p>
            </div>
